...
// src/lib/schemas.ts
import { z } from "zod";
var courseInputSchema, subtopicContentRequestSchema, diagnoseClarityRequestSchema, extractedSubtopicSchema, extractedTopicSchema, extractedUnitSchema, sourceExtractionSchema, generateRevisionRequestSchema, geminiSubtopicContentSchema, geminiDiagnoseClaritySchema, topicAnalysisRequestSchema, topicSubtopicItemSchema, keyConceptItemSchema, conceptualExampleItemSchema, commonConfusionItemSchema, conceptualClaritySchema, topicAnalysisResultSchema, conceptualClarityRequestSchema, generateQuestionRequestSchema, interactiveQuestionSchema, answerEvaluationRequestSchema, answerEvaluationResultSchema;
var init_schemas = __esm({
  "src/lib/schemas.ts"() {
    ...
    subtopicContentRequestSchema = z.object({
    ...
    diagnoseClarityRequestSchema = z.object({
    ...
    extractedSubtopicSchema = z.object({
    ...
    geminiSubtopicContentSchema = z.object({
      conceptual: z.object({
        quick: z.string(),
        standard: z.string(),
        deep: z.string(),
    ...
    geminiDiagnoseClaritySchema = z.object({
    ...
    topicAnalysisRequestSchema = z.object({
    ...
    topicSubtopicItemSchema = z.object({
      title: z.string().min(1),
      description: z.string().default(""),
      order: z.number().int().min(1)
    });
    keyConceptItemSchema = z.union([
      z.object({
        term: z.string().min(1),
    ...
    conceptualExampleItemSchema = z.union([
      z.object({
        title: z.string().min(1),
        explanation: z.string().min(1)
      }),
    ...
    commonConfusionItemSchema = z.union([
      z.object({
    ...
    conceptualClaritySchema = z.object({
    ...
    topicAnalysisResultSchema = z.object({
    ...
    conceptualClarityRequestSchema = z.object({
    ...
    generateQuestionRequestSchema = z.object({
    ...
    interactiveQuestionSchema = z.object({
    ...
    answerEvaluationRequestSchema = z.object({
    ...
    answerEvaluationResultSchema = z.object({
      isCorrect: z.boolean(),
...
var init_learningEngine = __esm({
  "src/server/learningEngine.ts"() {
    init_schemas();
    domainExpertExplanations = {
      "process scheduling": {
...
var geminiClient, aiResponseCache, quotaCooldownUntil, demandCooldownUntil;
var init_gemini = __esm({
  "src/server/gemini.ts"() {
    dotenv.config();
    geminiClient = null;
    aiResponseCache = /* @__PURE__ */ new Map();
    quotaCooldownUntil = 0;
    demandCooldownUntil = 0;
  }
});

// src/api-handlers/ai/subtopic-content.ts
var subtopic_content_exports = {};
__export(subtopic_content_exports, {
  default: () => handler
});
...
var init_subtopic_content = __esm({
  "src/api-handlers/ai/subtopic-content.ts"() {
    init_schemas();
    init_sourceEngine();
...
async function handler2(req, res) {
  ...
  if (check) {
    ...
    try {
      ...
      importStatus.gemini = !!GoogleGenAI2;
    } catch (e) {
      importStatus.geminiError = e.message;
    }
    try {
      const subtopic = await Promise.resolve().then(() => (init_subtopic_content(), subtopic_content_exports));
